...
static double frequency = 1e-3;            // CMS tracker reading frequency [Hz]
...
struct Hit // Holds info about one hit (position of a cluster)
{
    ...
    short int id;
    short int pclass;
    float eta; // Optional eta

    // Constructor without eta (default case)
    Hit(float r_, float z_, float phi_, short int id_, short int pclass_)
        : r(r_), z(z_), phi(phi_), id(id_), pclass(pclass_), eta(0)
    {
    }

    // Constructor with eta (if available)
    Hit(float r_, float z_, float phi_, float eta_, short int id_, short int pclass_)
        : r(r_), z(z_), phi(phi_), eta(eta_), id(id_), pclass(pclass_)
    {
    }
};
...
static const int MaxClasses = 6000;
static int pclass;
